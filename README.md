...
<p align="center">
  ...
  <a href="https://github.com/fzc13306355765-glitch/omniisle-write"><img alt="GitHub 公开仓库" src="https://img.shields.io/badge/GitHub-public-2ea44f"></a>
</p>
...
| 上下文管理 | 记忆库、关联文件、参考上文、5 个知屿内置提示词模板和用户模板 |
| 正文工作 | 剧情/字数双模式正文生成、富文本编辑、自动保存、历史版本、查找替换和排版 |
...
<table>
  ...
  <tr>
    ...
    <td width="50%">
      ...
      <br><strong>用户自备模型</strong><br>填写提供商、基础 URL 和 API Key，检索服务商的可用模型后自由选择；默认不长期保存密钥。
    </td>
  </tr>
</table>
...
浏览器打开终端显示的本机地址，默认是 <http://127.0.0.1:8081/>。不要直接双击 `index.html`：本机启动方式除了保证浏览器存储和模块正常加载，还会开启只监听 `127.0.0.1` 的模型转发通道，让未开放浏览器跨域的用户自备 API 也能正常使用。

第一次使用可以点击页面顶部的操作引导入口。教程会使用隔离的演示数据，不会调用模型，也不会写入真实作品。

社区版首次启动会在本机模板库中加入 5 个由知屿编写的公开模板：长篇小说大纲、单章细纲、拆书 A（结构节奏）、拆书 B（人物爽点）和番茄向爆款正文。“番茄向”只表示面向常见快节奏网文阅读习惯，不是平台官方模板，也不保证流量或成绩。用户创建、粘贴或导入的提示词仍只保存在当前浏览器，不会被写入仓库或覆盖。

## 配置自己的模型

1. 在写作页面点击“添加自己的模型”。
2. 选择提供商，或选择“自定义中转站”。
3. 填写基础 URL 和 API Key，点击“检索可用模型”，从返回的列表中选择后保存。
4. 在正文生成、工具模型或全文分析入口选择刚添加的模型。
5. 第一次请求前确认页面显示的目标域名，只向你信任的接口发送稿件。

模型检索使用服务商常见的 `/models` 接口。通过 `npm run serve` 运行时，浏览器请求会经由当前电脑上的本机转发通道发送，解决 OpenCode Zen 等服务未开放浏览器跨域的问题；本机通道不会把 Key 写入网址或日志。若部署成不带本机服务的纯静态网页，则只能直连允许浏览器跨域的模型商。

OpenCode Zen 请选择“OpenCode Zen（正文兼容）”。软件会自动使用 `https://opencode.ai/zen/v1`，并只展示官方标为 `chat/completions`、适合当前正文通道的模型；使用 `responses`、`messages` 或 Gemini 专用接口的模型暂不混入列表，避免选中后无法生成。完整的 `/models` 地址即使被粘贴到基础 URL，也会自动整理掉末尾 `/models`。

MiniMax 分为国内站和国际站：国内开放平台申请的 Key 请选择“MiniMax（国内）”，默认地址为 `https://api.minimaxi.com/v1`；国际站申请的 Key 请选择“MiniMax（国际）”，默认地址为 `https://api.minimax.io/v1`。两边 Key 与地址不能混用。

正文生成提供两种真实执行方式：“剧情”模式只请求一次，以剧情完整推进和自然收束为优先；“字数”模式在未达到目标时按计划继续请求，以接近目标字数为优先。两种模式都只调用当前选择的自备模型。单章目标上限为 20000 字；预计超过 5 次 API 请求时，软件会在开始前显示次数并再次确认。
...
- 社区版默认阻止未确认的外部请求；本机运行时，AI 稿件先经过仅监听 `127.0.0.1` 的本机转发再发送到使用者确认的模型地址，不经过知屿服务器。
...
- 已验证：Chrome、Microsoft Edge、核心本地写作、API Key 边界、AI优化链路、全文分析流程和知屿助手内容反馈中文输出
