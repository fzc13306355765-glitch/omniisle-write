...
> The local pre-publication release gates have passed. This directory is ready to be committed as the formal GitHub upload candidate. See [OPEN-SOURCE-STATUS.md](OPEN-SOURCE-STATUS.md) for the evidence.
...
- Newly configured API keys remain in the current page session by default. They are encrypted and persisted in the current browser only when the user explicitly enables “Remember API key.”
...
`npm test` checks the community runtime, local writing boundary, API-key persistence boundary, and full-text analysis flow. `npm run licenses:check` verifies the third-party inventory and notices; `npm run secrets:check` scans the current files and Git history for high-confidence secrets; `npm run audit:public` checks for hosted-service code, commercial implementation, extra networking, and missing assets. All checks must remain green before every public commit. See [Free GitHub repository setup](GITHUB-SETUP.md) for the owner-only settings.
...
Commercial licensing for closed-source integration or other terms is available separately from the rights holder. External contributions require acceptance of the [Contributor License Agreement](CLA.md). The project code license does not automatically cover logos, wallpapers, trademarks, or third-party components; see [ASSETS-LICENSES.md](ASSETS-LICENSES.md), [TRADEMARKS.md](TRADEMARKS.md), [NOTICE](NOTICE), and [THIRD_PARTY_NOTICES.md](THIRD_PARTY_NOTICES.md).
